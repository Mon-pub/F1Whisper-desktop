...
// Load the mock signing keypair generated by `apps/desktop/tools/build-electron.mjs` (and baked
// into the test build). Generate a separate, ephemeral keypair to produce signatures for the
// `wrong-signature` variant (its public key is not trusted by the build, so the app rejects OPPFs
// signed with it).
const OPPF_SIGNING_KEYPAIRS: OppfSigningKeypairs = {
    trusted: loadTrustedTestKeypair(),
    untrusted: generateKeypair(),
};
    ...
    try {
        ...
        switch (reqUrl) {
            ...
            case '/config.oppf': {
                ...
                sendOppf(res, statusCode, getOppfPayload(variant, OPPF_SIGNING_KEYPAIRS));
                return;
            }
